...
# Import the enhanced radar chart functions V2
try:
    from enhanced_radar_v2_with_fast_state import (
        RadarChartDataProviderV2,
        create_enhanced_radar_chart_with_units_v2,
        create_detail_chart_with_units_v2,
        get_performance_label
    )
    ENHANCED_V2_AVAILABLE = True
    print("✅ Enhanced radar chart functions V2 imported successfully")
except ImportError as e:
    print(f"⚠️  Enhanced V2 functions not available: {e}")
    ENHANCED_V2_AVAILABLE = False

# ⭐⭐⭐ THIS MUST BE HERE - AT MODULE LEVEL, NOT INSIDE if __name__ == '__main__' ⭐⭐⭐
# Initialize the enhanced data provider V2 if available
if ENHANCED_V2_AVAILABLE:
    try:
        ...
        print(f"✅ Enhanced data provider V2 initialized (Stage {provider.stage}/3)")
        print(f"📝 Display names: {len(provider.display_names_map)} mappings loaded")
    except Exception as e:
        print(f"⚠️  Enhanced provider V2 initialization failed: {e}")
        provider = None
        ENHANCED_V2_AVAILABLE = False
else:
    provider = None
...
# Alternative: Use a single master password for all counties
MASTER_PASSWORD = 'county_dashboard_2024'  # Set this to None to disable master password
...
if ENHANCED_V2_AVAILABLE:
    try:
        provider = RadarChartDataProviderV2(
            'sustainability_data.db',
            'display_names.csv'  # Optional display names file
        )
        print(f"✅ Enhanced data provider V2 initialized (Stage {provider.stage}/3)")
        print(f"📝 Display names: {len(provider.display_names_map)} mappings loaded")
    except Exception as e:
...
# REPLACE the database connection functions in county_secure_dashboard.py
# with these corrected versions:

def get_database_connection():
    """Get connection to sustainability database - CORRECTED"""
    # Use the SAME database file as your provider
    return sqlite3.connect('sustainability_data.db')  # Changed from 'county_health.db'

def get_all_counties():
    """Get list of all counties - Enhanced Version V2"""
    if ENHANCED_V2_AVAILABLE and provider:
        try:
            counties_df = provider.get_all_counties()
            if not counties_df.empty:
                print(f"✅ Using enhanced data provider V2: Found {len(counties_df)} counties")
                return counties_df
            else:
                print("⚠️  Enhanced provider returned no counties, falling back to original method")
        except Exception as e:
            print(f"⚠️  Enhanced provider failed: {e}, falling back to original method")
    
    # Fallback to original method - CORRECTED TABLE NAME
    ...
    try:
        # Try 'counties' table first (correct table for sustainability_data.db)
        counties_df = pd.read_sql("""
            SELECT fips as fips_code, county as county_name, state as state_code, state as state_name
            FROM counties 
            ORDER BY state, county
        """, conn)
        print(f"✅ Found {len(counties_df)} counties in 'counties' table")
    except Exception as e1:
        print(f"⚠️  Could not read from 'counties' table: {e1}")
        try:
            # Alternative: try raw_metrics table
            counties_df = pd.read_sql("""
                SELECT DISTINCT fips as fips_code, 
                       '' as county_name, 
                       '' as state_code, 
                       '' as state_name
                FROM raw_metrics
                ORDER BY fips
            """, conn)
            print(f"⚠️  Using raw_metrics table: {len(counties_df)} counties found")
        except Exception as e2:
            print(f"❌ Could not read from any table: {e2}")
            counties_df = pd.DataFrame(columns=['fips_code', 'county_name', 'state_code', 'state_name'])
    
    if not counties_df.empty:
        counties_df = counties_df.dropna(subset=['fips_code'])
...
def get_county_metrics(county_fips):
    """Get all metrics for a specific county - Enhanced Version V2"""
    if ENHANCED_V2_AVAILABLE and provider:
        try:
            # Use provider's method (this uses the correct database)
            county_info, structured_data = provider.get_county_metrics(county_fips)
            if not county_info.empty and structured_data:
                comparison_mode = provider.comparison_mode
                
                # Check if fast state comparisons are being used
                ...
                print(f"✅ Using enhanced V2 data with {speed_indicator} {comparison_mode} comparison")
                return county_info, structured_data
            else:
                print("⚠️  Enhanced provider returned no data")
                return pd.DataFrame(), {}
        except Exception as e:
    ...
    # If provider not available, return empty data
    print("❌ Enhanced provider not available - cannot load county data")
    return pd.DataFrame(), {}

def get_submetric_details(county_fips, top_level, sub_category):
    """Get detailed metrics - Enhanced Version V2"""
    if ENHANCED_V2_AVAILABLE and provider:
        try:
            # Use provider's built-in method
            details_df = provider.get_submetric_details(county_fips, top_level, sub_category)
            if not details_df.empty:
                comparison_mode = provider.comparison_mode
                speed_indicator = ""
                if comparison_mode == 'state':
                    speed_indicator = " ⚡" if provider.stage >= 3 else " ⏳"
                print(f"✅ Using enhanced V2 submetric details with{speed_indicator} {comparison_mode} comparison")
                return details_df
            else:
                print("⚠️  Enhanced provider returned no details")
                return pd.DataFrame()
        except Exception as e:
            print(f"❌ Enhanced submetric details failed: {e}")
            return pd.DataFrame()
    
    # If provider not available, return empty
    print("❌ Enhanced provider not available - cannot load submetric details")
    return pd.DataFrame()

def structure_metrics_from_columns_enhanced(county_data):
    """Structure metrics data from column-based format - Enhanced for new radar chart"""
    ...
    for col in metric_columns:
        ...
        if len(parts) >= 2:
            ...
            if top_level == 'Society':
                mapped_category = 'Society'
            elif top_level == 'Economy':
                mapped_category = 'Economy'
            elif top_level == 'Environment':
                mapped_category = 'Environment'
            else:
                if top_level in ['society']:
                    mapped_category = 'Society'
                elif top_level in ['economy']:
                    mapped_category = 'Economy'
                elif top_level in ['nature']:
                    mapped_category = 'Environment'
                else:
                    continue
            
            if mapped_category in structured:
                value = county_data[col].iloc[0]
                if pd.notna(value) and value != '':
                    try:
                        numeric_value = float(value)
                        if sub_category not in structured[mapped_category]:
                            structured[mapped_category][sub_category] = numeric_value
                    except:
                        pass
    
    return structured

def create_sector_based_radar_chart_enhanced_v2(county_data, county_name):
    """Enhanced radar chart V2 with human-readable names and state comparison"""
    if ENHANCED_V2_AVAILABLE and provider and provider.stage >= 2:
        try:
            county_fips = None
            counties_df = get_all_counties()
            for _, county in counties_df.iterrows():
                if f"{county['county_name']}, {county['state_code']}" == county_name:
                    county_fips = county['fips_code']
                    break
            
            if county_fips:
                return create_enhanced_radar_chart_with_units_v2(
                    county_data, county_name, provider, county_fips
                )
        except Exception as e:
            print(f"⚠️  Enhanced radar chart V2 failed: {e}, using fallback")
    
    # Fallback to enhanced original radar chart with interactive features
    return create_sector_based_radar_chart_original_enhanced(county_data, county_name)

def create_sector_based_radar_chart_original_enhanced(county_data, county_name):
    """Enhanced original radar chart with interactive features preserved"""
    if not county_data:
        return go.Figure()
    ...
    categories_config = {
        ...
        'Environment': {'color': '#059669', 'label': 'Environment', 'start_angle': 240, 'end_angle': 360}
    }
    ...
    if ENHANCED_V2_AVAILABLE and provider:
        ...
        stage_info = f"<br><sub>Percentile Rankings vs {comparison_label} Average • Click sub-measures for details</sub>"
    ...
    fig.update_layout(
        ...
        showlegend=False,
        title=dict(
            text=f"<b>{county_name} Sustainability Dashboard</b>{stage_info}",
            x=0.5,
...
def get_submetric_details(county_fips, top_level, sub_category):
    """Get detailed metrics - Enhanced Version V2 with fast state comparisons"""
    if ENHANCED_V2_AVAILABLE and provider:
        try:
            # Use the provider's built-in method which now has auto-detection for fast state comparisons
            details_df = provider.get_submetric_details(county_fips, top_level, sub_category)
            if not details_df.empty:
                comparison_mode = provider.comparison_mode
                speed_indicator = ""
                ...
                print("⚠️  Enhanced provider returned no details, falling back to original method")
        except Exception as e:
            print(f"⚠️  Enhanced submetric details failed: {e}, falling back to original method")
            ...
            if pd.notna(value) and value != '':
                try:
                    ...
                    details.append({
                        ...
                        'percentile_rank': 50,
                        'unit': '',
                        'display_name': metric_name.replace('KEYINDICATOR', '').title()
                    })
                except:
                    pass
...
def create_detail_chart_enhanced_v2(details_df, title):
    """Enhanced detail chart V2 with display names and comparison context"""
    if ENHANCED_V2_AVAILABLE and provider and not details_df.empty and 'display_name' in details_df.columns:
        try:
            return create_detail_chart_with_units_v2(details_df, title, provider.comparison_mode)
        except Exception as e:
            print(f"⚠️  Enhanced detail chart V2 failed: {e}, using fallback")
    ...
    fig.add_trace(go.Bar(
        ...
        orientation='h',
        marker=dict(
            color=details_df.get('percentile_rank', [50] * len(details_df)),
            colorscale='RdYlGn',
...
def create_access_denied_layout(error_message="Access Denied"):
    ...
    return html.Div([
        html.Div([
            ...
            html.Div([
                html.P("If you believe this is an error, please contact technical support.", 
...
def create_dashboard_layout(county_fips, county_info, structured_data):
    ...
    # Check provider stage for UI messaging
    stage_message = ""
    if ENHANCED_V2_AVAILABLE and provider:
        if provider.stage >= 3:
            stage_message = "⚡ Fast state comparisons enabled"
        elif provider.stage >= 2:
            stage_message = "💡 Run setup for fast state comparisons"
        
    # Create initial radar chart
    initial_radar_fig = create_sector_based_radar_chart_enhanced_v2(structured_data, county_name)
    
    return html.Div([
        # Header with enhanced title and comparison mode toggle
        html.Div([
            html.H1(f"{county_name} Sustainability Dashboard", 
                    className="text-3xl font-bold text-center text-gray-800 mb-2"),
            
            # Status indicator with comparison mode
            html.Div([
                html.Span(
                    f"✅ Stage {provider.stage}/3 Data Available" if ENHANCED_V2_AVAILABLE and provider and provider.stage > 0 else "❌ No Enhanced Data",
                    className=f"text-sm px-3 py-1 rounded-full text-white " + 
                             ("bg-green-600" if ENHANCED_V2_AVAILABLE and provider and provider.stage >= 2 else 
                              "bg-yellow-600" if ENHANCED_V2_AVAILABLE and provider and provider.stage == 1 else 
                              "bg-red-600")
                ),
                html.Span(
                    f"• {provider.comparison_mode.title()} Comparison" if ENHANCED_V2_AVAILABLE and provider else "Basic Mode",
                    className="text-xs text-gray-600 ml-2"
                ),
                html.Span(
                    f"• {len(provider.display_names_map)} Display Names" if ENHANCED_V2_AVAILABLE and provider else "",
                    className="text-xs text-gray-600 ml-2"
                ),
                html.Span(
                    f"• {stage_message}" if stage_message else "",
        ...
        html.Div([
            ...
            # Enhanced Summary section with comparison mode toggle
            html.Div([
                html.Div([
                ...
                html.Div([
                    ...
                    html.Div([
                        ...
                        html.Button(
                            "Compare with State", 
                            id='state-mode-btn',
                            className="w-full mb-2 px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700 transition-colors"
                        ),
                        html.Div(id='comparison-status', className="text-xs text-gray-600 mt-2"),
                        # Updated performance tip
                        html.Div([
                            html.Small(
                                "⚡ Fast state comparisons enabled!" if ENHANCED_V2_AVAILABLE and provider and provider.stage >= 3 
                                else "💡 State comparisons available (slower)" if ENHANCED_V2_AVAILABLE and provider and provider.stage >= 2
                                else "⚠️ Limited comparison features", 
                                className="text-xs italic " + 
                                         ("text-green-500" if ENHANCED_V2_AVAILABLE and provider and provider.stage >= 3 else
                                          "text-blue-500" if ENHANCED_V2_AVAILABLE and provider and provider.stage >= 2 else
                                          "text-yellow-500")
                            )
                        ], className="mt-2")
                    ])
                ...
                html.Div([
                    html.H3("Features", className="text-lg font-semibold mb-4"),
                    html.Div(id='feature-status')
                ], className="bg-white p-4 rounded-lg shadow-md mb-4"),
                ...
                html.Div([
                    ...
                    html.Button("Export Data", 
                               className="w-full mb-2 px-4 py-2 bg-gray-600 text-white rounded hover:bg-gray-700")
                ], className="bg-white p-4 rounded-lg shadow-md mb-4"),
                ...
                html.Div([
                    ...
                    html.Ul([
                        ...
                        html.Li("Society (Blue), Economy (Red), Environment (Green)"),
                        html.Li("Switch between National and State comparisons" if ENHANCED_V2_AVAILABLE and provider and provider.stage >= 2 else "Values show raw measurements"),
                        html.Li("Human-readable names for all metrics" if ENHANCED_V2_AVAILABLE and provider and len(provider.display_names_map) > 0 else "Enhanced features available after running Stage 2"),
                        html.Li("Hover for detailed information with units and context"),
                        html.Li("⚡ State comparisons are optimized for speed" if ENHANCED_V2_AVAILABLE and provider and provider.stage >= 3 
                               else "State comparisons use real-time calculations" if ENHANCED_V2_AVAILABLE and provider and provider.stage >= 2
                               else "Enhanced features require Stage 2+ setup", 
                               className="text-green-600" if ENHANCED_V2_AVAILABLE and provider and provider.stage >= 3 else "text-blue-600")
        ...
        html.Div([
            html.H2(id='detail-title', className="text-xl font-semibold mb-4", children="Click on a radar chart point to view detailed metrics"),
...
# Main App Layout - URL-based authentication
app.layout = html.Div([
...
def authenticate_and_display(url_search):
    """Main authentication callback - validates URL parameters and displays appropriate content"""
    if not url_search:
        return create_access_denied_layout("No access parameters provided. Please use the correct URL with county and key parameters.")
    ...
    try:
        ...
        return create_access_denied_layout("Invalid URL format. Please check your access URL.")
    ...
    try:
        ...
        if county_info.empty:
            return create_access_denied_layout(f"No data found for county {county_fips}. Please contact support.")
        
        # Set comparison mode if provider is available
        if ENHANCED_V2_AVAILABLE and provider:
            state_code = county_info.iloc[0]['state_code']
            provider.set_comparison_mode('national')  # Default to national
        
        print(f"✅ Authenticated access for county {county_fips}: {message}")
        return create_dashboard_layout(county_fips, county_info, structured_data)
        
    except Exception as e:
        print(f"❌ Error loading county data: {e}")
        return create_access_denied_layout(f"Error loading data for county {county_fips}: {str(e)}")

# Enhanced Callbacks (modified for secure access)
@app.callback(
...
def update_county_data_secure(comparison_mode, auth_data):
    """Update county data based on comparison mode for authenticated users"""
    print(f"🔄 update_county_data_secure called with mode: {comparison_mode}")
    ...
    if ENHANCED_V2_AVAILABLE and provider:
        if comparison_mode == 'state':
            # Get state code for the selected county
            counties_df = get_all_counties()
            ...
            if not county_row.empty:
                ...
                print(f"🏛️ Provider switched to state comparison mode for {state_code}")
        else:
            provider.set_comparison_mode('national')
            print(f"📊 Provider switched to national comparison mode")
    
    # Get county metrics (provider automatically chooses fast vs slow method)
    county_info, structured_data = get_county_metrics(county_fips)
    ...
    print(f"✅ Data updated for county {county_fips} in {comparison_mode} mode ({total_time:.2f}s total)")
...
def update_radar_chart_secure(county_data, county_info, auth_data):
    """Update radar chart for authenticated users"""
    if not auth_data or not auth_data.get('authenticated'):
        return go.Figure()
    ...
    return create_sector_based_radar_chart_enhanced_v2(county_data, county_name)
...
def update_comparison_mode_secure(national_clicks, state_clicks, current_mode, auth_data):
    """Update comparison mode for authenticated users"""
    if not auth_data or not auth_data.get('authenticated'):
        return current_mode
    ...
    print(f"🔄 Button clicked: {button_id}, Current mode: {current_mode}")
    
    if button_id == 'national-mode-btn':
        print("📊 Switching to national comparison")
        return 'national'
    elif button_id == 'state-mode-btn':
        print("🏛️ Switching to state comparison")
        return 'state'
...
def update_button_styles(comparison_mode):
    """Update button opacity based on active comparison mode"""
    print(f"🎨 Updating button styles for mode: {comparison_mode}")
    national_style = {
        'opacity': '1' if comparison_mode == 'national' else '0.6',
        'transform': 'scale(1.02)' if comparison_mode == 'national' else 'scale(1.0)'
    }
    state_style = {
        'opacity': '1' if comparison_mode == 'state' else '0.6', 
        'transform': 'scale(1.02)' if comparison_mode == 'state' else 'scale(1.0)'
    }
    return national_style, state_style
...
def update_summary_stats_secure(county_data, comparison_mode, auth_data):
    """Update summary stats for authenticated users"""
    if not auth_data or not auth_data.get('authenticated'):
    ...
    for category in ['Society', 'Economy', 'Environment']:
        if category in county_data and county_data[category]:
            ...
            # Add percentile context with comparison mode
            score_label = f"{avg_score}"
            if ENHANCED_V2_AVAILABLE and provider and provider.stage >= 2:
                score_label += "%"
                if avg_score >= 75:
                    context = f" (Top 25% {comparison_label.lower()})"
                elif avg_score >= 50:
                    context = f" (Above {comparison_label.lower()} avg)"
                elif avg_score >= 25:
                    context = f" (Below {comparison_label.lower()} avg)"
                else:
                    context = f" (Bottom 25% {comparison_label.lower()})"
                score_label += context
            ...
            stats_items.append(
                html.Div([
                    ...
                    html.Span(score_label, className="text-lg font-bold", style={'color': color})
...
def update_comparison_status_secure(comparison_mode, county_info, auth_data):
    """Update comparison status for authenticated users"""
    if not auth_data or not auth_data.get('authenticated'):
        return "Authentication required"
    
    if not ENHANCED_V2_AVAILABLE or not provider or not county_info:
        return "Enhanced features not available"
    
    if comparison_mode == 'national':
        return "📊 Comparing against all US counties"
    elif comparison_mode == 'state' and county_info:
        state = county_info.get('state_code', 'Unknown')
        speed_indicator = " ⚡" if provider.stage >= 3 else " ⏳"
        return f"🏛️ Comparing against counties in {state}{speed_indicator}"
...
def update_feature_status_secure(county_data, comparison_mode, auth_data):
    """Show what enhanced features are available for authenticated users"""
    if not auth_data or not auth_data.get('authenticated'):
        return [html.Li("🔒 Authentication Required", className="text-red-600")]
    
    features = []
    
    if ENHANCED_V2_AVAILABLE and provider and provider.stage >= 3:
        comparison_label = "National" if comparison_mode == 'national' else "State"
        features = [
            html.Li(f"✅ {comparison_label} Percentile Rankings", className="text-green-600"),
            html.Li("✅ Human-Readable Names", className="text-green-600"),
            html.Li("✅ Units in Hover Text", className="text-green-600"),
            html.Li("✅ Performance Context Labels", className="text-green-600"),
            html.Li("⚡ Fast State Comparisons", className="text-green-600"),
            html.Li("🔒 Secure County Access", className="text-blue-600"),
        ]
    elif ENHANCED_V2_AVAILABLE and provider and provider.stage >= 2:
        comparison_label = "National" if comparison_mode == 'national' else "State"
        features = [
            html.Li(f"✅ {comparison_label} Percentile Rankings", className="text-green-600"),
            html.Li("✅ Human-Readable Names", className="text-green-600"),
            html.Li("✅ Units in Hover Text", className="text-green-600"),
            html.Li("✅ Performance Context Labels", className="text-green-600"),
            html.Li("⏳ Slower State Comparisons", className="text-yellow-600"),
            html.Li("🔒 Secure County Access", className="text-blue-600"),
        ]
    elif ENHANCED_V2_AVAILABLE and provider and provider.stage >= 1:
        features = [
            html.Li("⚠️  Raw Values Only", className="text-yellow-600"),
            html.Li("❌ No Percentile Rankings", className="text-gray-400"),
            html.Li("❌ Limited Comparison Features", className="text-gray-400"),
            html.Li("💡 Run Stage 2 for Full Features", className="text-blue-600"),
            html.Li("🔒 Secure County Access", className="text-blue-600"),
        ]
    else:
        features = [
            html.Li("❌ No Enhanced Data", className="text-red-600"),
            html.Li("💡 Run Stage 1 & 2", className="text-blue-600"),
            html.Li("💡 Add display_names.csv", className="text-blue-600"),
            html.Li("🔒 Secure County Access", className="text-blue-600"),
        ]
    
    return html.Ul(features, className="text-sm space-y-1")
...
def handle_radar_click_secure(clickData, county_info, auth_data):
    """Handle radar chart clicks for authenticated users"""
    if not auth_data or not auth_data.get('authenticated'):
    ...
    try:
        ...
        if len(custom_data) >= 2:
            ...
            if not details_df.empty:
                comparison_context = ""
                if ENHANCED_V2_AVAILABLE and provider:
                    comparison_context = f" ({provider.comparison_mode.title()} Comparison)"
                
                title = f"{sub_category.title()} Metrics - {county_info['county_name']}, {county_info['state_code']}{comparison_context}"
                detail_fig = create_detail_chart_enhanced_v2(details_df, title)
...
if __name__ == '__main__':
    ...
    print(f"   • County-specific passwords: {len(COUNTY_PASSWORDS)} configured")
    print(f"   • Master password: {'Enabled' if MASTER_PASSWORD else 'Disabled'}")
    
    if COUNTY_PASSWORDS:
        print("   • Configured counties:")
        for fips, password in COUNTY_PASSWORDS.items():
            print(f"     - County {fips}: {password}")
    
    print(f"\n📋 Access URL Format:")
    print(f"   • http://yourdomain.com/?county=FIPS_CODE&key=PASSWORD")
    print(f"   • Example: http://localhost:8050/?county=39133&key=portage2024")
    ...
    if ENHANCED_V2_AVAILABLE and provider:
        ...
        if provider.stage == 3:
            print("✅ Full Enhanced Features Available:")
            print("   • Human-readable metric names")
            print("   • National vs State comparison modes")
            print("   • ⚡ Fast state comparisons")
            print("   • Percentile rankings with comparison context")
            print("   • Enhanced hover information with units")
            print("   • Performance labels with comparison context")
            print("   • Interactive comparison mode switching")
        elif provider.stage == 2:
            print("✅ Enhanced Features Available:")
            print("   • Human-readable metric names")
            print("   • National vs State comparison modes")
            print("   • ⏳ Slower state comparisons")
            print("   • Percentile rankings with comparison context")
            print("   💡 Run setup_fast_state_comparisons() for instant state comparisons")
        elif provider.stage == 1:
            print("⚠️  Basic Enhanced Features Available:")
            print("   • Human-readable metric names")
            print("   • Raw data visualization")
            print("   💡 Run Stage 2 for percentile rankings and comparisons")
        else:
            print("❌ No enhanced data found")
            print("   💡 Run Stage 1 and Stage 2 for full functionality")
    else:
        print("❌ Enhanced V2 features not available")
        print("   💡 Ensure enhanced_radar_integration_v2.py is in the same directory")
        print("   💡 Run Stage 1 and Stage 2 for full functionality")
        print("   💡 Create display_names.csv for human-readable names")
    
    print(f"\n🔒 Security Features:")
    print(f"   • URL-based authentication")
    print(f"   • County-specific access control")
    print(f"   • No complex login system required")
    print(f"   • Password validation on every page load")
